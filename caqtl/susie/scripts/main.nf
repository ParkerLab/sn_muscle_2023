...
workflow{
	loci = channel_glob(params.loci)
		.map{it -> it[0].tokenize("--").plus([it[1]])}
...
process adjust_phenotypes {
	...
	input:
	tuple val(cluster), val(covtype), path(caqtl), path(bed), path(index), path(cov), path(covlist) 
	...
	"""
	less $caqtl | grep -v gene_name | awk '{if ((\$NF==0)) print \$1}' > ${prefix}.pheno;
	QTLtools correct --bed $bed --cov $cov --normal --include-covariates $covlist --out ${prefix}.adjusted.bed  --include-phenotypes ${prefix}.pheno;
	split -l 500 ${prefix}.pheno ${prefix}.pheno-list.
	rm ${prefix}.pheno
	"""
