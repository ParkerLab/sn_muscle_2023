...
parser.add_argument('--barcode-corrections-file', type=str, required=True, help = 'file to save barcode corrections for readnames')
parser.add_argument('--corrected-barcode-counts-file', type=str, required=True, help = 'file with counts of each barcode across 4 readgroups')
...
corrected_barcode_counts = collections.Counter()

fixes = dict()
with open(args.barcode_corrections_file, 'w+') as bcorrections:
    for fastq in args.fastq:
        outputfastq = fastq.replace(".fastq.gz",".corrected.fastq")
        logging.info(f'Processing fastq file {fastq}')
        read_count = 0
        with gzip.open(fastq, 'rt') as f:
            with open(outputfastq, 'w+') as outf:
                for read_name, uncorrected_barcode, phred in FastqGeneralIterator(f):
                    read_count += 1
                    if read_count % 1000000 == 0:
                        logging.info(f'Processed {read_count} reads')
                    if uncorrected_barcode not in good_barcodes:
                        ...
                        bcorrections.write(f'{read_name}\t{uncorrected_barcode}\t{corrected_barcode}\n')
                    else:
                        corrected_barcode = uncorrected_barcode
                    outf.write(f"@{read_name}\n{corrected_barcode}\n+\n{phred}\n")
                    corrected_barcode_counts[corrected_barcode] += 1

# print(corrected_barcode_counts)
with open(args.corrected_barcode_counts_file, 'w+') as f:
    f.write('\n'.join([f"{key}\t{value}" for key, value in corrected_barcode_counts.items()]))
