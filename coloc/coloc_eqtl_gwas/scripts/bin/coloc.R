...
option_list <- list(
    ...
    make_option(c("--eqtl"), type = "character", help = "space-separated eqtl SuSiE outputs to run coloc with"),
...
run.coloc = function(gwas.res, eqtl, rsdf, prefix){

    load(eqtl) ## loads out_ds
    ## eQTL snp names are not rsids, fix that.
    ...
    if(length(out_ds$sets$cs)){
        ...
        print(glue("{prefix} - no eqtl sets"))
    }
}
...
ca.file.list = unlist(strsplit(opts$eqtl, ","))

lapply(ca.file.list, function(cafile){
    eqtl.locus = gsub(".susie.Rda", "", basename(cafile))
    prefix = glue("{gwas.locus}__{eqtl.locus}")
